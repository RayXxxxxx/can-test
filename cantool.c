...
static void usage(void)
{
    ...
    printf("DOC:http://velep.com/archives/1177.html");
    printf("\n");
 ...
 */
int set_bitrate(const int port, const int bitrate)
{
#define TX_QUEUE_LEN		4096 // 使用足够多的发送缓存
    FILE *fp;
    char str[50];
    sprintf(str, "sudo ip link set can%d type can bitrate %d", port, bitrate);
...
int open_can(const int port, const int bitrate)
{
#ifndef WIN32
    char	l_c8Command[64] = {0};

    sprintf(l_c8Command, "sudo ip link set can%d up", port);
    int rtrn = system(l_c8Command);
    if ( rtrn != 0 )
    {
        printf("CAN%d down return value = %d", port, rtrn);
        return rtrn;
    }
...
int main(int argc, char **argv)
{
    ...
    if (set_bitrate(port, bitrate) < 0 )// 操作CAN之前，先要设置波特率
    {
        printf("CAN$d set bitrate to $bitrate failed", port, bitrate);
	return -1;
    }
